...
@section('content')
    <div class="container-fluid">
        <form method="POST" action="{{ route('admin.registrations.store') }}" onsubmit="return mySubmit()" autocomplete="off">
            @csrf
                            ...
                            <div class="form-group m-0">
                                ...
                                <select name="user_id" class="form-control @error('user_id') is-invalid @enderror">
                                    ...
                                    @foreach ($students as $student)
                                        <option value="{{ $student->id }}">{{ $student->name }} ({{ $student->local }})</option>
                                    @endforeach
                                </select>
                                @error('user_id')
                                    <small class="text-red line-height">{{ $message }}</small>
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="payment" value="nao" checked="true">
                                    <label class="form-check-label">Aguardando pagamento da matrícula.</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="payment" value="sim">
